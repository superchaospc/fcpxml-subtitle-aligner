...
1. Locate the CLI with `command -v fcpxml-aligner`. If unavailable and this repository is available, resolve the caller-supplied repository root and discover build products from that root instead of guessing an architecture-specific path. Check both `$(swift build -c release --show-bin-path)/fcpxml-aligner` and `$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/fcpxml-aligner`; require the selected candidate to be a regular executable file. These cover the native SwiftPM layout and the universal layout, whose repository-relative location is normally `.build/apple/Products/Release/fcpxml-aligner`. If neither candidate exists, run `swift build -c release --product fcpxml-aligner`, obtain the native directory again with `swift build -c release --show-bin-path`, and use its executable. Never hardcode a user path. If no local binary is available, use the pinned GitHub Release fallback only when network access and download are explicitly authorized: canonical repository `https://github.com/superchaospc/fcpxml-subtitle-aligner`, tag `v1.0.0`, exact asset `fcpxml-aligner-v1.0.0-macos-universal.tar.gz`, and exact companion `https://github.com/superchaospc/fcpxml-subtitle-aligner/releases/download/v1.0.0/SHA256SUMS`. Require the matching expected filename entry in SHA256SUMS; abort if the release, asset, entry, or checksum is unavailable or mismatched. Extract in a temporary directory and use only the expected fcpxml-aligner binary. Do not execute downloads automatically unless agent or user scope permits. A missing binary is a controlled error.
